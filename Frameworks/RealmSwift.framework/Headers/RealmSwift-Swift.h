...
/// Override to designate a property as the primary key for an Object subclass. Only properties of type String and Int can be designated as the primary key. Primary key properties enforce uniqueness for each value whenever the property is set which incurs some overhead. Indexes are created automatically for primary key properties. :returns: Name of the property designated as the primary key, or nil if the model has no primary key.
...
/// Return an array of property names for properties which should be indexed. Only supported for string and int properties. :returns: Array of property names to index.
...
/// Returns the value for the property identified by the given key. :param: key The name of one of the receiver's properties. :returns: The value for the property identified by key.
- (id __nullable)valueForKey:(NSString * __nonnull)key;

/// Sets the property of the receiver specified by the given key to the given value. :param: value The value for the property identified by key. :param: key   The name of one of the receiver's properties.
